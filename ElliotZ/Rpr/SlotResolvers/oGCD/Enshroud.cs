...
public class Enshroud : ISlotResolver
{
    ...
    public int Check()
    {
        ...
        if (Qt.Instance.GetQt("»êÒÂ") == false) { return -98; }
        ...
        if (!Qt.Instance.GetQt("ÇãÐº×ÊÔ´") && !Core.Me.HasAura(AurasDef.IdealHost))  // ignore all if dump qt is set
        {
            if (!Qt.Instance.GetQt("µ¥»êÒÂ") && RprHelper.Shroud < 100)
            {
                if (!DeadZoneCheck())
                {
                    return -6;
