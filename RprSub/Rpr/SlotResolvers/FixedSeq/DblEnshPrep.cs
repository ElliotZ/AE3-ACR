...
public class DblEnshPrep : ISlotSequence {
  ...
  public int StartCheck() {
    ...
    if (SpellsDef.ArcaneCircle.GetSpell().Cooldown.TotalMilliseconds > PreAcEnshTimer) return -6;
  ...
  private static void Step2(Slot slot) {
    ...
    if (Qt.Instance.GetQt("爆发药") && ItemHelper.CheckCurrJobPotion()) {
      ...
      SlotAction.WaitType wait = BattleData.Instance.NumBurstPhases == 0
                                     ? SlotAction.WaitType.WaitForSndHalfWindow
                                     : SlotAction.WaitType.None;
      slot.Add(new SlotAction(wait, 0, SpellsDef.ArcaneCircle.GetSpell()));
    }
