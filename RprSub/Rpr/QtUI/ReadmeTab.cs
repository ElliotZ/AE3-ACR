...
public static class ReadmeTab {
  private static readonly InfoBox _box = new() {
      ...
      ContentsAction = () => {
        ImGui.Text("欢迎使用EZRpr！开始使用之前请确保阅读以下QT选项说明以及设置页面，如果有问题" 
                 + "欢迎DC留言！");
        ...
        ImGui.Text("真北优化：开了之后挥割以及挥割变化的缢杀/绞决爪技能会在保证不溢出红条的情况"
                 + "下等待身位正确或者真北施放后才打出。逻辑比较保守，基本不会导致资源溢出，正常情况"
                 + "下可以启用后隐藏。");
        ImGui.Bullet();
        ImGui.SameLine();
        ImGui.Text("自动突进：在关闭勾刃QT的情况下或者循环中为了止损禁止打勾刃的情况下，如果足够远"
                 + "离Boss则会自动向目标方向施放地狱入境。“足够远离”的标准是打不到目标，并且施放地狱入"
                 + "境之后的落点不会超出近战范围。");
        ImGui.Bullet();
        ImGui.SameLine();
