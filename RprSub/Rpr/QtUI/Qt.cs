...
public static class Qt {
  ...
  private static readonly List<(string name, string ENname, bool defval, string tooltip)> _qtKeys = [
      ("爆发药", "Pot", false, ""),
      ("爆发药2分", "Pot2min", true, ""),
      ("起手", "Opener", true, ""),
      ("单魂衣", "SingleShroud", false, 
       "不给爆发留魂衣，爆发只会有1个大丰收送的魂衣"),
      ("神秘环", "ArcaneCircle", true, ""),
      ("大丰收", "PlenHar", true, ""),
      ("印记", "DeathsDesign", true, 
       "死亡之影和死亡之涡"),
      ("灵魂割", "SoulSkill", true, 
       "灵魂切割以及AOE灵魂钐割"),
      ("挥割/爪", "Bloodstalk", true, 
  ...
  private static readonly List<(string name, string ENname, IHotkeyResolver hkr)> _hkResolvers = [
      ...
      ("神秘纹", "Crest",
       new HotKeyResolver(SpellsDef.ArcaneCrest, 
                          SpellTargetType.Self, 
                          _forceNextSlots)),
      ("LB", "LB", new HotKeyResolver_LB()),
      ("亲疏", "Armslength",
       new HotKeyResolver(SpellsDef.ArmsLength, 
                          SpellTargetType.Self, 
                          _forceNextSlots)),
      ("内丹", "SecondWind",
       new HotKeyResolver(SpellsDef.SecondWind, 
                          SpellTargetType.Self, 
                          _forceNextSlots)),
      ("浴血", "BloodBath",
       new HotKeyResolver(SpellsDef.Bloodbath, 
                          SpellTargetType.Self, 
                          _forceNextSlots)),
      ("牵制", "Feint", 
       new HotKeyResolver(SpellsDef.Feint, 
                          useHighPrioritySlot: _forceNextSlots)),
      ("真北", "TrueNorth",
       new HotKeyResolver(SpellsDef.TrueNorth, 
                          SpellTargetType.Self, 
                          _forceNextSlots)),
