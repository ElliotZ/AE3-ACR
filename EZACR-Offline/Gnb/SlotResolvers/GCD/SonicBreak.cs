...
public class SonicBreak : ISlotResolver
{
    ...
    public int Check()
    {
        ...
        if (!16153u.GetSpell().IsReadyWithCanCast())
        {
            return -50;
        }
        if (16153u.RecentlyUsed() && !Qt.Instance.GetQt("优先音速破"))
        {
            return -5;
        }
        if (16146u.GetSpell().IsReadyWithCanCast() && Qt.Instance.GetQt("子弹连") && !Qt.Instance.GetQt("优先音速破"))
        {
            return -6;
        }
        ...
        if (!Qt.Instance.GetQt("音速破"))
        {
            return -11;
        }
        if (16146u.CoolDownInGCDs(2) && Qt.Instance.GetQt("子弹连") && !Qt.Instance.GetQt("优先音速破") && Core.Resolve<MemApiSpell>().GetLastComboSpellId() == 16139 && Core.Resolve<JobApi_GunBreaker>().Ammo == 0)
        {
            return -9;
        }
