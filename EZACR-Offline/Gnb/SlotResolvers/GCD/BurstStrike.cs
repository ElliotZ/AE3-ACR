...
public class BurstStrike : ISlotResolver {
  ...
  public static Spell GetSpell() {
    if ((Qt.Instance.GetQt("AOE") || Qt.Instance.GetQt("强制变命运")) && (Core.Me.Level >= 72)) {
      ...
      if (((nearbyEnemyCount2 >= 2) || (!Qt.Instance.GetQt("强制变命运") && (nearbyEnemyCount >= 2)))
  ...
  public int Check() {
    ...
    if ((Core.Resolve<JobApi_GunBreaker>().Ammo > 0) && Qt.Instance.GetQt("倾泻爆发")) {
      return 10;
    }
    ...
    if (!Qt.Instance.GetQt("爆发")) {
      if ((Core.Me.Level >= 88) && (Core.Resolve<JobApi_GunBreaker>().Ammo == 3)) {
        ...
        if (Core.Resolve<MemApiSpell>().GetLastComboSpellId() == 16139) return 123;
      }
      ...
      if ((Core.Me.Level < 88) && (Core.Resolve<JobApi_GunBreaker>().Ammo == 2)) {
        ...
        if (Core.Resolve<MemApiSpell>().GetLastComboSpellId() == 16139) return 125;
      }
    ...
    if ((GnbSettings.Instance.ACRMode == "绝亚2G") && Core.Me.HasAura(1831u)) {
      if (16146u.CoolDownInGCDs(1)) return -4;

      if (Core.Resolve<JobApi_GunBreaker>().Ammo > 0) return 4;
    }
    ...
    if (Core.Me.Level >= 91) {
      ...
      if ((Core.Resolve<JobApi_GunBreaker>().Ammo > 0)
       && (!25760u.CoolDownInGCDs(5)
        || (!Qt.Instance.GetQt("倍攻") && (!16146u.CoolDownInGCDs(2) || !Qt.Instance.GetQt("子弹连"))))
    ...
    if (GnbSettings.Instance.ACRMode == "Normal") {
      if ((Core.Resolve<JobApi_GunBreaker>().Ammo > 0) && Core.Me.HasAura(1831u)) return 99;
    ...
    if (Core.Me.Level < 88) {
      if (Core.Resolve<JobApi_GunBreaker>().Ammo < 2) return -100;
    ...
    if ((Core.Me.Level >= 88) && (Core.Resolve<JobApi_GunBreaker>().Ammo == 3)) {
      ...
      if (Core.Resolve<MemApiSpell>().GetLastComboSpellId() != 16139) return -5;
    }
