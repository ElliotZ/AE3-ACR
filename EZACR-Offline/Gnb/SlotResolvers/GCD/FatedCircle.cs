...
public class FatedCircle : ISlotResolver {
  ...
  public int Check() {
    ...
    //    return -152;
    //}
    if (Core.Me.Level < 72) return -2;
    if (!16163u.GetSpell().IsReadyWithCanCast()) return -5;

    int nearbyEnemyCount = TargetHelper.GetNearbyEnemyCount(Core.Me, 8, 8);
    if (nearbyEnemyCount < 2) return -4;

    if (Core.Me.Level < 88) {
      if (Core.Resolve<JobApi_GunBreaker>().Ammo < 2) return -1;
    } else if (Core.Resolve<JobApi_GunBreaker>().Ammo < 3) {
      return -8;
    }

    if (!Qt.Instance.GetQt("AOE")) return -3;
    if (Qt.Instance.GetQt("倾泻输出") && (Core.Resolve<JobApi_GunBreaker>().Ammo > 0)) return 2;
    return 0;
  }
