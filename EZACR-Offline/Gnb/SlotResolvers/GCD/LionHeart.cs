...
public class LionHeart : ISlotResolver {
  ...
  public int Check() {
    ...
    if (!36937u.GetSpell().IsReadyWithCanCast()) return -10;

    if (!Qt.Instance.GetQt("爆发")) return -30;

    if (!Qt.Instance.GetQt("狮心连")) return -40;

    if (!Core.Me.HasAura(3840u) && (Core.Resolve<MemApiSpell>().GetLastComboSpellId() == 36939)) {
      return -4;
    }

    if ((Core.Resolve<MemApiSpell>().GetLastComboSpellId() == 36937)
     || (Core.Resolve<MemApiSpell>().GetLastComboSpellId() == 36938)) {
      return 5;
    }

    if (Core.Me.HasMyAuraWithTimeleft(3840u, 3000)) return 39;

    if (16138u.CoolDownInGCDs(2)
     && !Qt.Instance.GetQt("无视无情")
     && (Core.Resolve<MemApiSpell>().GetLastComboSpellId() != 36937)
     && (Core.Resolve<MemApiSpell>().GetLastComboSpellId() != 36938)) {
      return -25;
    }
