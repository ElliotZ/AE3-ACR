...
public class Opener100_2gcd_mk2 : IOpener, ISlotSequence, IScript
{
    public List<Action<Slot>> Sequence { get; } = [Step0, Step1, Step2, Step3, Step4, Step5, Step6, Step7, Step8, Step9];
    ...
    {
        LogHelper.Print("KKxb绝枪", "开始2GCD起手改");
    ...
    public void InitCountDown(CountDownHandler countDownHandler)
    {
        ...
        if (GnbSettings.Instance.起手给MT刚玉)
        {
            countDownHandler.AddAction(2000, 25758u, SpellTargetType.Pm2);
        }
        ...
        if (GnbSettings.Instance.倒计时开铁壁)
        {
            countDownHandler.AddAction(GnbSettings.Instance.铁壁Time, 7531u);
        }

        LogHelper.Print($"角色职能: {AI.Instance.PartyRole}, 盾姿状态: {Core.Me.HasAura(1833u)}, 自动盾姿配置: {GnbSettings.Instance.倒计时自动盾姿}, ST关盾配置: {GnbSettings.Instance.倒计时是否ST关盾姿}");
