...
public class DangerZone : ISlotResolver {
  ...
  public int Check() {
    ...
    if (Core.Me.Level > 79) return -3;

    if (Core.Me.Level < 18) return -4;

    if (!16144u.GetSpell().IsReadyWithCanCast()) return -88;

    if (Core.Me.HasAnyAura(GnbHelper.ContBuffs)) return -99;

    if (16138u.CoolDownInGCDs(2) && !Qt.Instance.GetQt("无视无情")) return -5;
    ...
    if (!Qt.Instance.GetQt("爆发")) return -6;

    if (!Qt.Instance.GetQt("爆破领域")) return -7;
