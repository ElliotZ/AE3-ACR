...
public class Bloodfest : ISlotResolver {
  ...
  public int Check() {
    ...
    if (Core.Resolve<JobApi_GunBreaker>().Ammo > 0) return -1;

    if (!Qt.Instance.GetQt("爆发")) return -2;

    if (!Qt.Instance.GetQt("血壤")) return -7;

    if (!16164u.GetSpell().IsReadyWithCanCast()) return -3;

    if (Qt.Instance.GetQt("自动拉怪")) return -15;
