...
public class NoMercy : ISlotResolver {
  ...
  public int Check() {
    ...
    if (!16138u.GetSpell().IsReadyWithCanCast()) return -5;
    ...
    if (Qt.Instance.GetQt("倾泻爆发")) return 3;

    if (!Qt.Instance.GetQt("爆发")) return -3;

    if (!Qt.Instance.GetQt("无情")) return -42;

    if (Qt.Instance.GetQt("无情后半")) {
      if (GCDHelper.GetGCDCooldown() > 700) return -8;
    ...
    if (GCDHelper.GetGCDCooldown() < 300) return -9;
