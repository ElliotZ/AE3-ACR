...
public class BowShock : ISlotResolver
{
    ...
    public int Check()
    {
        ...
        if (!16159u.GetSpell().IsReadyWithCanCast())
        {
            return -3;
        }

        if (Core.Me.Level < 62 || !16159u.IsUnlock())
        {
            return -5;
        }

        if (Core.Me.Distance(Core.Me.GetCurrTarget(), DistanceMode.IgnoreHitbox) > 5f)
        {
            return -3;
        }
        ...
        {
            if (Core.Me.HasAura(1831u))
            {
                return 0;
            }

            if (Qt.Instance.GetQt("无视无情"))
            {
                return 7;
            }
