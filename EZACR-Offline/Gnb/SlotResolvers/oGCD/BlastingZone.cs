...
public class BlastingZone : ISlotResolver {
  ...
  public int Check() {
    ...
    if (Core.Me.Level < 80) return -3;

    if (!16165u.GetSpell().IsReadyWithCanCast()) return -4;

    if (16138u.CoolDownInGCDs(2) && !Qt.Instance.GetQt("无视无情")) return -88;
    ...
    if (!Qt.Instance.GetQt("爆发")) return -1;

    if (!Qt.Instance.GetQt("爆破领域")) return -4;
