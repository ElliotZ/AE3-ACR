...
public class Camouflage : ISlotResolver {
  ...
  public int Check() {
    if (!16140u.GetSpell().IsReadyWithCanCast()) return -3;
    ...
    if (!Qt.Instance.GetQt("自动拉怪")
     && (!16148u.GetSpell().IsReadyWithCanCast() || 16148u.RecentlyUsed())) {
      if (Core.Me.CurrentHpPercent() > GnbSettings.Instance.伪装阈值) return -13;
      ...
      if ((Core.Me.CurrentHpPercent() <= GnbSettings.Instance.伪装阈值)
       && (Core.Me.HasAura(1834u, 1000) || Core.Me.HasAura(1191u, 1000))) {
        return 3;
      }
    }
