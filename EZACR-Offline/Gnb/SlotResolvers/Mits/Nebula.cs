...
public class Nebula : ISlotResolver
{
    ...
    public int Check()
    {
        if (!16148u.GetSpell().IsReadyWithCanCast())
        {
            return -3;
        }
        ...
        if (num >= 3)
        {
            if (7531u.RecentlyUsed() || 16140u.RecentlyUsed())
            {
                return -8;
            }
        ...
        if (Core.Me.GetCurrTarget().IsBoss() && Core.Me.Level > 50)
        {
            return -10;
        }
        ...
        if (!Qt.Instance.GetQt("自动拉怪"))
        {
            if (Core.Me.CurrentHpPercent() < GnbSettings.Instance.星云阈值 && !Core.Me.HasAnyAura(auras, 3000))
            {
                return 3;
            }

            if (Core.Me.CurrentHpPercent() < GnbSettings.Instance.星云阈值 && (!Core.Me.HasAura(1832u, 1000) || !Core.Me.HasAura(1191u, 1000)))
            {
                return 4;
            }
