...
public class Rampart : ISlotResolver {
  public SlotMode SlotMode { get; } = SlotMode.OffGcd;

  public int Check() {
    if (!7531u.GetSpell().IsReadyWithCanCast()) return -3;
    ...
    if (Core.Me.HasAura(1834u) && Core.Me.HasAnyAura(auras, 2000)) return -3;
    ...
    if (num >= 3) {
      if (!16148u.GetSpell().IsReadyWithCanCast() || 16148u.RecentlyUsed()) {
        if (Core.Me.HasAura(1834u) || Core.Me.HasAnyAura(auras, 2000)) return -8;
    ...
    if (Qt.Instance.GetQt("自动拉怪")) return -11;

    if (((ulong)Core.Me.GetCurrTarget().MaxHp > (ulong)((long)Core.Me.MaxHp * 12L))
     && (Core.Me.Level > 50)) {
      return -12;
    }

    if (!Qt.Instance.GetQt("自动拉怪")
     && (!16148u.GetSpell().IsReadyWithCanCast() || 16148u.RecentlyUsed())) {
      if ((Core.Me.CurrentHpPercent() < GnbSettings.Instance.铁壁阈值)
       && !Core.Me.HasAnyAura(auras, 2000)) {
        return 3;
      }

      if ((Core.Me.CurrentHpPercent() < GnbSettings.Instance.铁壁阈值)
       && (!Core.Me.HasAura(1834u, 1000) || Core.Me.HasAura(1832u))) {
        return 4;
      }

      if (Core.Me.CurrentHpPercent() > GnbSettings.Instance.铁壁阈值) return -13;
    }
