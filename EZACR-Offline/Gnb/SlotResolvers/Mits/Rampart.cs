...
public class Rampart : ISlotResolver
{
    ...
    public int Check()
    {
        if (!7531u.GetSpell().IsReadyWithCanCast())
        {
            return -3;
        }
        ...
        {
            if (Core.Me.CurrentHpPercent() < GnbSettings.Instance.铁壁阈值 && !Core.Me.HasAnyAura(auras, 2000))
            {
                return 3;
            }

            if (Core.Me.CurrentHpPercent() < GnbSettings.Instance.铁壁阈值 && (!Core.Me.HasAura(1834u, 1000) || Core.Me.HasAura(1832u)))
            {
                return 4;
            }
