...
public class Aurora : ISlotResolver {
  ...
  public int Check() {
    if (!16151u.GetSpell().IsReadyWithCanCast()) return -3;

    if (Core.Me.HasAura(1835u)) return -8;

    var auras = new List<uint> { 3255u, 409u, 810u };
    if (!Qt.Instance.GetQt("减伤")) return -5;

    if (16151u.RecentlyUsed()) return -15;
    ...
    if (Core.Me.CurrentHpPercent() > GnbSettings.Instance.极光阈值) return -9;
