...
public class Reprisal : ISlotResolver {
  ...
  public int Check() {
    ...
    if (!Qt.Instance.GetQt("雪仇")) return -100;

    if (!7535u.GetSpell().IsReadyWithCanCast()) return -50;

    if (TargetHelper.targetCastingIsBossAOE(Core.Me.GetCurrTarget(), 10000)) return 2;
    ...
    if (num >= 3) {
      if (!16140u.GetSpell().IsReadyWithCanCast()
       && !7531u.GetSpell().IsReadyWithCanCast()
       && !16148u.GetSpell().IsReadyWithCanCast()) {
