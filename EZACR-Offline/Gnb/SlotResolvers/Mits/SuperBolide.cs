...
public class SuperBolide : ISlotResolver {
  ...
  public int Check() {
    if (!16152u.GetSpell().IsReadyWithCanCast()) return -3;
    ...
    if (!Qt.Instance.GetQt("自动超火")) return -5;

    if ((Core.Me.CurrentHpPercent() < GnbSettings.Instance.超火流星阈值)
     && Core.Me.HasAura(3255u)
     && Core.Me.HasAura(810u)
     && Core.Me.HasAnyAura(auras, 3000)) {
      return -3;
    }

    if (Core.Me.CurrentHpPercent() > GnbSettings.Instance.超火流星阈值) return -5;
