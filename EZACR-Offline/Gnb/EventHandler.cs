...
public class EventHandler : IRotationEventHandler {
  ...
  public async Task OnPreCombat() {
    ...
    if (GnbSettings.Instance.ACRMode != "Normal") {
      ...
      Qt.Instance.NewDefault("自动拉怪", true);
      Qt.Instance.SetQt("自动拉怪", true);
    }
  ...
  public void OnBattleUpdate(int currTimeInMs) {
    ...
    if (!Core.Me.IsMoving() && Qt.Instance.GetQt("自动拉怪")) {
      if ((DateTime.Now - BattleData.Instance.LastFalseTime).TotalSeconds
       >= GnbSettings.Instance.自动拉怪停止时间) {
